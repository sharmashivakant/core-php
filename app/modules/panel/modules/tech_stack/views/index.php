<div id="control-container">
    <div id="button-holder">
        <a href="{URL:panel/tech_stack/add}" onclick="load('panel/tech_stack/add');" class="btn add">
            <i class="fas fa-plus-circle"></i> Add New Tech
        </a>
        <div class="clr"></div>
    </div>
    <h1>
        <i class="fas fa-users"></i>Tech Stack
    </h1>
    <hr/>
        ...
        <?php if (isset($this->list) && is_array($this->list) && count($this->list)) { ?>
            <?php foreach ($this->list as $item) { ?>
                <tr>
                    ...
                    <td>
                        <img src="<?= _SITEDIR_; ?>data/tech_stack/<?= $item->image; ?>" height="20">&nbsp;
                        <span style="margin-left: 50px;"><?= $item->name; ?><span>
                    </td>
                    <td class="option__buttons">
                        <a href="{URL:panel/tech_stack/edit/<?= $item->id; ?>}" class="icon fa fa-fw fa-pencil-alt tooltip" title="Edit"></a>
                        <a href="{URL:panel/tech_stack/delete/<?= $item->id; ?>}" class="icon fa fa-fw fa-trash tooltip" title="Delete"></a>
                    </td>
                </tr>
            <?php } ?>
        <?php } ?>
