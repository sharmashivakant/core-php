...
class Controller
{
    ...
    public function processing()
    {
        ...
        if ($action !== false) {
            $redirectUrl = false; // Moved url

            if (array_key_exists(User::getRole(), $permission[$action])) { // Check role permissions
                if ($permission[$action][User::getRole()]['allow'] === false) {
                    if (array_key_exists('redirect', $permission[$action][User::getRole()])) // Use role('admin') redirect
                        $redirectUrl = $permission[$action][User::getRole()]['redirect'];
