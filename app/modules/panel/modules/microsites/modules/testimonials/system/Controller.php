    ...
    public function addAction()
    {
        ...
        if ($this->startValidation()) {
            ...
            if ($this->isValid()) {
                ...
                if ($data['image']) {
                    if (!File::copy('data/tmp/' . $data['image'], 'data/microsite/testimonials/' . $data['image']))
...
//                    print_data('data/microsite/testimonials/' . $data['image']);
                    if (File::copy('data/tmp/' . $data['image'], 'data/microsite/testimonials/' . $data['image'])) {
                        File::remove('data/microsite/testimonials/' . $this->view->testimonial->image);
                    } else
                        print_data(error_get_last());
                }
...
//                    Request::addResponse('redirect', false, url('panel', 'microsites', 'testimonials', 'edit', $id));
    ...
    public function deleteAction()
    {
        ...
        if ($result) {
//            $this->session->set_flashdata('success', 'Testimonial created successfully.');
//            Request::addResponse('redirect', false, url('panel', 'microsites_testimonials', 'edit', $insertID));
        } else {
            Request::returnError('Database error');
        }
